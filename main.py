...
def read_bookseries_header(file):
    ...
    bookseries_match = regex.search(r'bookseries="(?P<bookseries_id>\w+)"[\s>]', first_line, flags=regex.UNICODE)
    booknumber_match = regex.search('book_number="(?P<book_number>\w+)"[\s>]', first_line, flags=regex.UNICODE)
    return bookseries_match.group('bookseries_id'), booknumber_match.group('book_number')
...
def extract(args):
    ...
    xml_reader = StreamingXMLReader(input_file.name,
                                    bookseries.convert_xml_to_dict)

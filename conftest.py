...
def test_geo_db():
    """
    Creates a test database which is copy of the normal geo database.
    Should be used with Peewee's test_database functionality:
    ...
    Note that this database is created once per session, so changes
    to it will be in effect in other tests which use the test db.
    If you need a clean test db, improve this fixture but take care
    to make it efficient by only restoring the db on tests that need
    it (set autouse=False) or modifying this to use in-memory db.
    """
    path = './temp'
    if not os.path.exists(path):
        os.makedirs(path)

    # Copy the db to temp location in favor of using in-memory copy.
    # While in-memory would be likely faster, the copying process
    # seems slightly more complex and perhaps prone to errors. Look
    # this SO-post and comments to solution for possible
    # complications:

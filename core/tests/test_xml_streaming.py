...
class TestStreamingXMLReader:
    ...
    def should_stream_xml_with_multiple_elements_correctly(self):
        def xml_to_extractor(person_element):
            ...
            return {**person_element.attrib, 'raw': _raw.text,
                    'conllu': _conllu.text}

        reader = StreamingXMLReader(mock_file(XML_WITH_COMPLEX_PERSON_ENTRIES),
                                    xml_to_extractor,
                                    chunk_size=2)
        entries = [entry for entry in reader.read_entry_from_xml()]

        expected_data = (('TESTIKÄS, TESTAAJA', ('foo', 'fooconllu')),

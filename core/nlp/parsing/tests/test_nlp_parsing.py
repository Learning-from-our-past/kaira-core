...
class TestNLPParsing:
    ...
    @pytest.fixture(autouse=True, scope='session')
    def bookseries(self):
        return bootstrap.setup_extraction_framework_for_bookseries(
            'siirtokarjalaiset', 'extractors/bookseries/', None
        )
    ...
    def transformed_test_data(self, bookseries, nlp_test_dir, raw_test_data):
        ...
        preprocessed_path = run_xml_data_transformation(
            bookseries, raw_test_data, output_path
        )
    ...
    def nlp_test_data(self, nlp_test_dir, transformed_test_data):
        output_path = '{}/raw_test_data'.format(nlp_test_dir)
        nlp_data_path = parse_through_fdp_and_output_file(
            transformed_test_data, output_path
        )
        return nlp_data_path

    @pytest.fixture(autouse=True, scope='session', name='raw_and_nlp_xml')
    def xml_with_raw_and_nlp_test_data(
        self, nlp_test_dir, raw_test_data, nlp_test_data
    ):
        output_path = '{}/raw_and_nlp_test_data.xml'.format(nlp_test_dir)
        with open(output_path, mode='w', encoding='utf8') as output_file:
            nlp_and_raw_xml_path = add_conllu_data_to_xml(
                raw_test_data, nlp_test_data, output_file
            )
        return nlp_and_raw_xml_path

    class TestTextPreprocessing:
        @pytest.mark.skip()
        def should_preprocess_xml_file_and_output_text_file(
            self, transformed_test_data
        ):
            assert os.path.isfile(transformed_test_data)

        @pytest.mark.skip()
        def should_separate_person_entries_in_preprocessed_file_with_comments(
            self, transformed_test_data
        ):
    ...
    class TestNLPDataGeneration:
        @pytest.mark.skip()
        def should_process_preprocessed_data_with_fdp_and_output_file(
            self, nlp_test_data
        ):
            assert os.path.isfile(nlp_test_data)

        @pytest.mark.skip()
        def should_have_person_entries_separated_by_comments_in_nlp_data_file(
            self, nlp_test_data
        ):
    ...
    class TestNLPDataJoiningToXML:
        ...
        @pytest.mark.skip()
        def should_have_person_elements_with_raw_and_conllu_subelements_in_xml_file(
            self, raw_and_nlp_xml
        ):

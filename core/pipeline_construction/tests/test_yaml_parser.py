...
from extractors.bookseries.karelians.extractors.name_extractor import NameExtractor
...
def should_build_and_run_pipeline(parser):
    pipeline = parser.build_pipeline_from_yaml(
        'core/pipeline_construction/tests/test_config.yaml'
    )
    ...
    assert (
        results[0]['primaryPerson']['profession']['professionName'] == 'maanviljelijä'
    )
...
class TestDependencyConfiguration:
    def should_resolve_dependencies_and_produce_correct_results(self, parser):
        pipeline = parser.build_pipeline_from_yaml(
            'core/pipeline_construction/tests/dependency_test_config.yaml'
        )
        results = pipeline.process(test_data)

        assert (
            results[0]['dependent']
            == 'This is from standalone extractor: Standalone extractor'
        )


class TestSubPipelineCreation:
    def should_build_subpipeline_correctly_and_extract_data_using_given_configuration(
        self, parser
    ):
        pipeline = parser.build_pipeline_from_yaml(
            'core/pipeline_construction/tests/subpipeline_config.yaml'
        )
        results = pipeline.process(test_data)
    ...
    def should_build_pipelines_of_arbitrary_depth(self, parser):
        pipeline = parser.build_pipeline_from_yaml(
            'core/pipeline_construction/tests/deep_subpipeline_config.yaml'
        )
        ...
        assert (
            results['outerExtractor']['innerExtractor']['innerExtractor']['message']
            == 'Level 3'
        )
...
class OuterExtractor(BaseExtractor):
    ...
    def _extract(self, entry, extraction_results, extraction_metadata):
        ...
        if self._sub_extraction_pipeline:
            results, metadata = self._sub_extraction_pipeline.process(
                {'text': entry['text']}
            )
...
class DependentExtractor(BaseExtractor):
    ...
    def _extract(self, entry, extraction_results, extraction_metadata):
        result = 'This is from standalone extractor: {}'.format(
            self._deps['outer']['outerExtractor']['message']
        )
        return self._add_to_extraction_results(

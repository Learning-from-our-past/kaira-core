...
class TestBaseExtractor:
    ...
    class TestPreprocess:
        ...
        def should_not_run_preprocess_and_post_process_if_they_are_not_implemented(
            self, th
        ):
    ...
    class TestMetadata:
        def should_reset_metadata_collector_after_extraction(self, extractor):
            ...
            assert extractor.metadata_collector.get_metadata() == {
                'errors': {}
            }  # Collector should be empty again

        def should_provide_starting_position_and_get_previous_cursor_location_correctly_when_required(
            self, th
        ):
            extractor = th.setup_extractor(
                MockExtractor(NoPreAndPostProcessesExtractor)
            )
            entry = {'text': 'test string entry'}
            extraction_results = {
                'mock2': {
    ...
    class TestDependencyResolving:
        ...
        def dep_extractors(self, result_map):
            ...
            return {
                'standalone': [extractor1, extractor2],
                'dependent': dependent_extractor,
            }
        ...
        def should_set_ids_of_required_extractors(self, dep_extractors):
            ...
            assert dep_extractors['dependent']._required_dependencies == [
                id(dep_extractors['standalone'][0]),
                id(dep_extractors['standalone'][1]),
            ]

        def should_assign_dependency_names_to_a_corresponding_dependencies(
            self, dep_extractors
        ):
            ...
            assert dep_extractors['dependent']._expected_dependencies_names == [
                'standalone',
                'standalone2',
            ]

        def should_resolve_dependencies_to_deps_object_before_extraction(
            self, dep_extractors
        ):
            extractors = [
                dep_extractors['standalone'][0],
                dep_extractors['standalone'][1],
                dep_extractors['dependent'],
            ]
            ...
            assert dep_extractors['dependent']._deps == {
                'standalone': {'mock': 'SOME RESULT'},
                'standalone2': {'mock': 'SOME RESULT'},
            }
            # And it should have been used to generate the results
            assert (
                results[0]['mock2deps']
                == 'This is from standalone extractor: SOME RESULT'
            )

        def should_store_extraction_results_to_the_extraction_results_map(
            self, result_map, dep_extractors
        ):
            extractors = [
                dep_extractors['standalone'][0],
                dep_extractors['standalone'][1],
            ...
            assert result_map.get_results(id(dep_extractors['standalone'][0])) == {
                'mock': 'SOME RESULT'
            }
            assert result_map.get_results(id(dep_extractors['standalone'][1])) == {
                'mock': 'SOME RESULT'
            }

        def should_raise_error_if_incorrect_amount_of_dependencies_were_provided(
            self, result_map
        ):
            extractor1 = MockExtractor()
            extractor1.set_extraction_results_map(result_map)
            ...
            with pytest.raises(RequiredDependenciesAreMissing):
                dependent_extractor.set_required_dependencies(
...
class NoPreAndPostProcessesExtractor(BaseExtractor):
    ...
    def __init__(self, cursor_location_depends_on=None, options=None):
        super(NoPreAndPostProcessesExtractor, self).__init__(
...
class SimpleExtractorForDeps(BaseExtractor):
    ...
    def __init__(self, cursor_location_depends_on=None, options=None):
        super(SimpleExtractorForDeps, self).__init__(
            cursor_location_depends_on, options
        )
    ...
    def _extract(self, entry, extraction_results, extraction_metadata):
        result = 'This is from standalone extractor: {}'.format(
            self._deps['standalone']['mock']
        )
        return self._add_to_extraction_results(

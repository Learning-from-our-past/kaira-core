...
class TestSexExtraction:
    ...
    def should_extract_male_when_most_names_are_male_names_but_there_is_a_female_name(self, sex):
        name_string = 'Tuomas Juuso Maria'
        assert sex.find_sex(name_string) == 'Male'

    def should_extract_female_when_most_names_are_female_names_but_there_is_a_male_name(self, sex):
        name_string = 'Joonas Virpi Maria'
        assert sex.find_sex(name_string) == 'Female'

    def should_extract_none_when_there_are_equal_numbers_of_male_and_female_names(self, sex):
        name_string = 'Maria Tuomas'
        assert sex.find_sex(name_string) is None

    def should_raise_exception_when_person_has_name_but_no_female_or_male_names_can_be_determined(self, sex):

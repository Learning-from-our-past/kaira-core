...
from extractors.bookseries.karelians.tests.duplicate_deletion import mock_xml_data


@pytest.mark.skipif(
    not import_util.find_spec('ssdeep'),
    reason='Tests cannot be ran without ssdeep which is difficult to install on MacOS.',
)
class TestDuplicateDeletion:
    ...
    def should_correctly_delete_exact_duplicate_and_only_return_one_entry(
    ...
    def should_correctly_delete_nearly_exact_duplicate_when_birthdate_is_missing(
        self, duplicate_deleter
    ):
    ...
    def should_correctly_delete_very_similar_duplicate_when_name_and_birthdate_fail_to_match_identify_duplicate(
        self, duplicate_deleter
    ):
    ...
    def should_correctly_delete_duplicate_but_keep_longer_text_from_duplicate_entry(
        self, duplicate_deleter
    ):
        book = [etree.fromstring(mock_xml_data.DUPLICATED_PERSON_BAD_DOB_AND_NAME)]
        dupes_deleted = duplicate_deleter.delete_duplicate_persons(book)
    ...
    def should_correctly_delete_duplicate_with_bad_birthdate_but_very_similar_name(
        self, duplicate_deleter
    ):
    ...
    def should_correctly_delete_duplicates_across_multiple_books_and_not_touch_nonduplicates(
        self, duplicate_deleter
    ):
        books = [

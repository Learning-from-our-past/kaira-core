...
from core.utils import text_utils as text_utils
...
class HostessExtractor(BaseExtractor):
    ...
    def __init__(self, cursor_location_depends_on=None, options=None):
        ...
        self.HOSTESS_NAME_PATTERN = (
            r"emäntä(?:nä)?(?:\svuodesta\s\d\d\d\d)?(?P<name>[A-ZÄ-Öa-zä-ö\.\s-]+),"
        )
    ...
    def _find_hostess_name(self, text):
        hostess_name_match = regexUtils.safe_search(
            self.HOSTESS_NAME_PATTERN, text, self.HOSTESS_OPTIONS
        )

...
_year = Or([(Word(hyphens).suppress() + Word(nums).setResultsName('moved_out')), Word(nums).setResultsName('moved_in') + Word(hyphens).suppress(), Word(nums).setResultsName('moved_out')])
...
_place_name = (Word(alphas + scandinavian_letters) + Optional(Literal('mlk'))).setResultsName('place')
_place_with_year_data = _place_name('place') + punctuation + _year_information('year_information')
_place_with_municipality = Optional(_place_name('municipality') + punctuation)
_place_or_place_with_year_data = Or([_place_name('place'), _place_with_year_data]) + punctuation

# Rule for getting place and municipality name if one is set for location
_place_or_place_and_municipality = Or([_place_or_place_with_year_data, _place_with_municipality + _place_or_place_with_year_data])

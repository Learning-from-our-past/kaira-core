...
class TestPlaceNameNormalizationWithManualLists:
    ...
    def should_return_fixed_name(self):
        result = place_name_cleaner.try_to_normalize_place_name_with_known_aliases('Rääkkylässä')
        assert result == 'Rääkkylä'

    def should_return_place_name_unmodified_if_it_was_not_found_from_list(self):
        result = place_name_cleaner.try_to_normalize_place_name_with_known_aliases('Testimaa')
        assert result == 'Testimaa'

    def should_return_region_if_it_is_asked(self):
        place_name, region = place_name_cleaner.try_to_normalize_place_name_with_known_aliases('Rääkkylässä', True)
        assert place_name == 'Rääkkylä'
        assert region == 'other'

    def should_return_region_as_None_if_it_was_not_found(self):
        place_name, region = place_name_cleaner.try_to_normalize_place_name_with_known_aliases('Testimaa', True)
...
class TestPlaceNameNormalizationWithPlaceList:
    ...
    def should_skip_if_place_has_coordinate_data(self):
        ...
        result = place_name_cleaner.normalize_place_name_with_known_list_of_places(entry)
    ...
    def should_find_closest_match_with_jw_if_stem_was_not_found_from_list_and_cache_result(self, metadata_collector):
        ...
        result = place_name_cleaner.normalize_place_name_with_known_list_of_places(entry, metadata_collector)
...
class TestNormalizePlaceUtilityFunction:
    ...
    def should_not_use_name_list_if_manual_list_had_match(self, mocker):
        mocker.patch(
            'book_extractors.common.postprocessors.place_name_cleaner.normalize_place_name_with_known_list_of_places')
        entry = {
